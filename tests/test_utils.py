...
def test_filter_count(
    sample_transactions_df: pd.DataFrame, start_date: datetime, end_date: datetime, expected_count: int
) -> None:
...
def test_filter_preserves_columns(sample_transactions_df: pd.DataFrame) -> None:
    ...
    result: pd.DataFrame = filter_transactions_by_date(
        sample_transactions_df, datetime(2021, 8, 1), datetime(2021, 8, 2)
    )
...
def test_filter_preserves_data_integrity(sample_transactions_df: pd.DataFrame) -> None:
    ...
    result: pd.DataFrame = filter_transactions_by_date(
        sample_transactions_df, datetime(2021, 8, 1), datetime(2021, 8, 3)
    )
...
def test_filter_successful_transaction_parametrized(
    statuses: List[str], sums: List[float], expected_count: int
) -> None:
    """Параметризованный тест с разными комбинациями статусов"""
    df: pd.DataFrame = pd.DataFrame({"Статус": statuses, "Сумма операции": sums, "Категория": ["Еда"] * len(statuses)})
    ...
    if expected_count > 0:
        ...
        expected_sums: List[float] = [sums[i] for i, status in enumerate(statuses) if status.upper() == "OK"]
...
@pytest.mark.parametrize(
    "amounts, currencies, rates, expected_amounts",
    [
...
def test_convert_transactions_to_rub(
    amounts: List[float], currencies: List[str], rates: Dict[str, float], expected_amounts: List[float]
) -> None:
    """Тест конвертации нескольких транзакций"""
    df: pd.DataFrame = pd.DataFrame({"Валюта операции": currencies, "Сумма операции": amounts})
...
def test_prepare_transactions_for_services_without_dates() -> None:
    ...
    df: pd.DataFrame = pd.DataFrame({"Сумма операции": [100, 200], "Категория": ["A", "B"]})
...
def test_get_currency_rates_various_cases(api_response: Dict[str, Any], expected_rates: Dict[str, float]) -> None:
...
def test_get_stock_prices_price_is_zero_int() -> None:
    ...
    mock_logger.warning.assert_called_with("Цена AMZN: $225.00 (заглушка - нет данных в API)")
...
def test_get_stock_prices_http_error() -> None:
    ...
    mock_logger.warning.assert_called_with("Цена AAPL: $270.00 (заглушка - ошибка HTTP)")
...
def test_get_stock_prices_network_error() -> None:
    ...
    mock_logger.warning.assert_called_with("Цена GOOGL: $142.00 (заглушка - ошибка подключения)")
...
def test_get_stock_prices_general_exception() -> None:
    ...
    assert result[0]["price"] == 270.0
    mock_logger.warning.assert_called_with("Цена AAPL: $270.00 (заглушка - ошибка подключения)")
...
def test_get_stock_prices_json_decode_error() -> None:
    ...
    mock_logger.warning.assert_called_with("Цена NVDA: $188.00 (заглушка - ошибка подключения)")
